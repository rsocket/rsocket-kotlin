 ...
 *
 *
 * RSocket impl strongly assumes that each Frame is encoded with the length. This is not true for
    ...
    override fun receive(): Flowable<Frame> {
        return inbound.receive()
                .map { buf ->
                    ...
                    val length = wrappedBuffer(ByteArray(frameLengthSize))
                    encodeLength(length, 0, buf.readableBytes())
