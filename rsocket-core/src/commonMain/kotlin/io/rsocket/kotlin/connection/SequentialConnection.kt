...
internal class SequentialConnection(
    ...
    private inner class EstablishmentContext : ConnectionEstablishmentContext(frameCodec) {
        override suspend fun sendFrame(frame: Source): Unit = connection.sendFrame(streamId = 0, frame)
        override suspend fun receiveFrameRaw(): Source? = connection.receiveFrame()
    }
    ...
    private inner class Outbound(streamId: Int) : OperationOutbound(streamId, frameCodec) {
        ...
        override suspend fun sendFrame(frame: Source): Unit = connection.sendFrame(streamId, frame)
    }
