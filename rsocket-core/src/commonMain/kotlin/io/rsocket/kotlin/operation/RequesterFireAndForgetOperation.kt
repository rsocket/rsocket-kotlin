...
internal class RequesterFireAndForgetOperation(
    private val requestSentDeferred: CompletableDeferred<Unit>,
) : RequesterOperation() {
    override val type: RSocketOperationType get() = RSocketOperationType.FireAndForget

    override suspend fun execute(outbound: OperationOutbound) {
        requestSentDeferred.complete(Unit)
    }

    override fun receiveProcessingError(cause: Throwable) {
        requestSentDeferred.completeExceptionally(cause)
    }
