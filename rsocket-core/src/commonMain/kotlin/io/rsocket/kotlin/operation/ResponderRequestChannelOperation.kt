                ...
                val requestFlow = payloadFlow { strategy, initialRequest ->
                    ...
                    val senderJob = launch {
                        try {
                            ...
                            // ignore error if outbound was closed - TODO: recheck
                            if (this@coroutineScope.isActive && outbound.isClosed) return@launch
                            // send cancel only if the operation is active
                            if (this@coroutineScope.isActive) withContext(NonCancellable) { outbound.sendCancel() }
                            throw cause
                        }
                    }
