...
internal class OperationStateStorage<T : Any>(private val isClient: Boolean) {
    ...
    fun acceptStream(id: Int, state: T): Boolean {
        if (isClient.xor(id % 2 == 0)) return false
        return synchronized(lock) {
            val isValid = id !in storage
            if (isValid) storage[id] = state
            isValid
        }
    }

    fun removeStream(id: Int): T? = synchronized(lock) {
        storage.remove(id)
    }

    fun getStream(id: Int): T? = synchronized(lock) {
        storage[id]
    }
