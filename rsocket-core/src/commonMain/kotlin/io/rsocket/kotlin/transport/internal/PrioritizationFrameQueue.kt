...
private val selectFrame: suspend (ChannelResult<Source>) -> ChannelResult<Source> = { it }

@RSocketTransportApi
public class PrioritizationFrameQueue(buffersCapacity: Int) {
    private val priorityFrames = channelForCloseable<Source>(buffersCapacity)
    private val normalFrames = channelForCloseable<Source>(buffersCapacity)
    ...
    private fun channel(streamId: Int): SendChannel<Source> = when (streamId) {
        0    -> priorityFrames
        else -> normalFrames
    }

    public suspend fun enqueueFrame(streamId: Int, frame: Source): Unit = channel(streamId).send(frame)

    public fun tryDequeueFrame(): Source? {
        // priority is first
    ...
    public suspend fun dequeueFrame(): Source? {
