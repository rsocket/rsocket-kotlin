...
public sealed interface CompositeMetadata : Metadata {
    ...
    override fun Sink.writeSelf() {
        entries.forEach {
            ...
            it.content.withLength { source, length ->
                writeInt24(length.toInt()) // write metadata length
    ...
    public class Entry(public val mimeType: MimeType, public val content: Source) {
        public constructor(metadata: Metadata) : this(metadata.mimeType, metadata.toSource())
    }
    ...
    public companion object Reader : MetadataReader<CompositeMetadata> {
        ...
        override fun Source.read(): CompositeMetadata {
            ...
            while (!exhausted()) {
                ...
                val packet = readSource(length.toLong())
