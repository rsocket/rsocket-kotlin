...
public sealed interface LocalClientTarget : RSocketClientTarget {
    public val serverName: String
}

public sealed interface LocalClientTransport : RSocketTransport<String, LocalClientTarget> {
    public fun target(server: LocalServerInstance): LocalClientTarget

    public companion object Factory : RSocketTransportFactory<
            String,
            LocalClientTarget,
            LocalClientTransport,
            LocalClientTransportBuilder>(::LocalClientTransportBuilderImpl)
}

public sealed interface LocalClientTransportBuilder :
...
private class LocalClientTransportImpl(
    ...
    override fun target(address: String): LocalClientTarget = LocalClientTargetImpl(
        serverName = address,
        coroutineContext = coroutineContext.supervisorContext(),
        server = LocalServerInstanceImpl.get(address)
    )

    override fun target(server: LocalServerInstance): LocalClientTarget = LocalClientTargetImpl(
        serverName = server.serverName,
        coroutineContext = coroutineContext.supervisorContext(),
        server = server as LocalServerInstanceImpl
    )
}
