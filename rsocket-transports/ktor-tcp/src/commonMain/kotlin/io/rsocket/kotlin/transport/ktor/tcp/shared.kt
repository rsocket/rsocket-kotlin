...
//TODO: check how ktor CIO engine works with selector
...
internal fun KtorTcpSelector?.createFor(parentContext: CoroutineContext): SelectorManager {
    ...
    // TODO: investigate why we need Job(job) here, because otherwise joining transport job is broken
    if (manage) Job(parentContext.job).invokeOnCompletion { selectorManager.close() }
    return selectorManager
}

@Suppress("SuspendFunctionOnCoroutineScope")
internal suspend inline fun <T> CoroutineScope.launchCont(crossinline block: suspend () -> T): T {
    ensureActive()
    return suspendCancellableCoroutine { cont ->
        ...
        job.invokeOnCompletion {
            if (it != null && cont.isActive) cont.resumeWithException(it)
        }
        cont.invokeOnCancellation {
            job.cancel("Cancelled by user", it)
        }
    }
}
